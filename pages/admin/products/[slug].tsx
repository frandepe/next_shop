...
const ProductAdminPage: FC<Props> = ({ product }) => {
  ...
  useEffect(() => {
    ...
    //* Con esto destruimos el watch de arriba para que deje de dispararse
    return () => subscription.unsubscribe();
  }, [watch, setValue]);

  const onChangeSize = (size: string) => {
    //* El orden en que se guardan los size lo hacemos en el backend
  ...
  const onSubmit = async (form: FormData) => {
    // console.log(form);
    if (form.images.length < 2) return alert("Minimo 2 imágenes");
    setIsSaving(true);
    try {
